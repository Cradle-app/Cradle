...
export class TelegramCommandsPlugin extends BasePlugin<Config> {
  ...
  async generate(
    ...
    // Commands Composer
    this.addFile(output, 'commands-composer.ts', this.generateCommandsComposer(config), 'backend-lib');

    // Webhook Handler
    if (config.deliveryMethod === 'webhook') {
      this.addFile(output, 'telegram-webhook-route.ts', this.generateWebhookHandler(config), 'backend-routes');
    ...
    if (config.deliveryMethod === 'polling') {
      this.addFile(output, 'telegram-bot.ts', this.generatePollingScript(config), 'root');
    }
