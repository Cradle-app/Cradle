...
function renderClaudeMd(
  ...
  const { project, network } = blueprint.config;
  const isWeb2Primary = blueprintUsesWeb2PrimaryScaffold(sortedNodes);
  const defaultDescription = isWeb2Primary
  ...
  const implOrder = sortedNodes
    .map((n, i) => {
      ...
      return `${i + 1}. **${name}** (\`${n.type}\`) — see \`\.nskills/components/${n.type}--${shortId(n.id)}.md\``;
    })
    .join("\n");
  ...
  const specLinks = sortedNodes
    .map((n) => {
      const registryEntry = PLUGIN_REGISTRY[n.type];
      const name = registryEntry?.name || humanizeKey(n.type);
      return `- [${name}](\.nskills/components/${n.type}--${shortId(n.id)}.md)`;
    })
    .join("\n");
...
*Generated by [[N]skills](https://www.nskills.xyz) — Compose N skills for your ${isWeb2Primary ? "Web2" : "Web3"} project.*
`;
}
...
export class SkillsRepoGenerator {
  ...
  async generate(
    ...
    try {
      ...
      fs.mkdirSync("/output/\.nskills/components", { recursive: true });
      ...
      for (const node of sortedNodes) {
        ...
        fs.writeFileSync(
          `/output/\.nskills/components/${filename}`,
          componentMd
        );
      }
      ...
      // Project
      fs.writeFileSync(
        "/output/\.nskills/project.md",
        renderProjectMd(blueprint)
      );

      // Architecture
      fs.writeFileSync(
        "/output/\.nskills/architecture.md",
      ...
      const dedupedEnvVars = dedupeEnvVars(allEnvVars);
      fs.writeFileSync(
        "/output/\.nskills/environment.md",
        renderEnvironmentMd(dedupedEnvVars)
      );

      // Dependencies
      fs.writeFileSync(
        "/output/\.nskills/dependencies.md",
        renderDependenciesMd(deps)
      );

      // Scripts
      fs.writeFileSync(
        "/output/\.nskills/scripts.md",
        renderScriptsMd(allScripts)
      );

      // Integration map
      fs.writeFileSync(
        "/output/\.nskills/integration-map.md",
